...
* [4-5 느슨한 그리고 엄격한 동등 비교](#4-5-느슨한-그리고-엄격한-동등-비교)
* [4-6 추상 관계 비교](#4-6-추상-관계-비교)
* [4-7 정리하기](#4-7-정리하기)
...
이렇게 falsy도 아니고 희귀 사례도 아닌 강제변환 결과는 안전하고 합리적으로 설명할 수 있다.

#### 근본부터 따져보자

암시적 강제변환을 깊숙이 파보니 정말 말도 안되는 것들이 몇 가지 발견되었다.

하지만 예상을 벗어난 정도를 기준으로 하면 강제변환이 심각한 문제가 될 경우는 7가지다.

그럼 그 많은 강제변환의 좋은 부분을 활용하는 동시에 몇몇 나쁜 부분을 어떻게 잘 피할수 있을까?
...
이들 중 처음 4개는 == false 비교와 연관된다. 다시 말하지만 이런 의미 없는 비교는 절대 하지 말자. 그다음 3개를 보자.
...
이런 강제변환 코드를 사용할 일이 있을까? == [] 식으로 불리언 평가를 할 경우는 극히 드물다. 대신, 다음과 같이 == "" 나 == 0을 사용하지 않을까?
...
우연히 doSomething(0)나 doSomething([])으로 호출하면 "어이쿠"라고 할 것이다. 다른 예제를 보자.
...
역시 마찬가지로 doSomething("", 0)이나 doSomething([] "")로 호출하면 문제가 된다.

결국 강제변환 때문에 골탕 먹을 경우의 수가 있고 함정에 빠지지 않으려면 주의할 필요가 있다. 그렇지만 크도 베이스 전체를 통틀어 그럴 만한 코드가 나올 가능성은 매우 희박하다.

#### 암시적 강제변환의 안전한 사용법

불행한 사고를 미연에 방지하는 차원에서 몇 가지 동등 비교 원칙을 제시하겠다.
...
결국 == 냐 === 냐 하는 문제는 "동등 비교 시 강제변환을 허용할 거냐 말 거냐"와 본질적으로 같다.

강제변환(명시적/암시적)의 강력한 파워를 효과적이고 안전하게 사용하는 방법을 잘 익혀서 주변 동료들 역시 잘 인도해주기 바란다.

NOTE: 강제변환에 상처받지 않을 또 한 가지 방법은 typeof 연산자다. typeof 연산은 항상 7가지 문자열 중 하나(공백문자""는 없다)를 반환하므로(1장 타입 참고), 값의 타입을 체크한다고 암시적 강제변환과 문제가 될 일은 전혀 없다. typeof x == "function", typeof x === "function" 둘다 100% 안전하다. 무턱대고 ===를 남발하지 말자.

다음 그림은 동등 비교의 모든 조합을 나타낸 테이블로 깃허브 유저인 Alex Dorey가 작성했다.
...
덜 관심 받지만 a < b 비교 과정에서 어떤 일들이 벌어지는지 잘 알아둬야 한다.

ES5 11.8.5 '추상적 관계 비교' 알고리즘은 비교 시 피연산자 모두 문자열(후반부) 일 때와 그 외의 경우(전반부), 두 가지로 나뉜다.

이 알고리즘은 먼저 두 피연산자에 대해 ToPrimitive 강제변환을 실시하는 것으로 시작한다. 그 결과, 어느 한쪽이라도 문자열이 아닐 경우 양쪽 모두 ToNumber로 강제변환하여 숫자 값으로 만들어 비교한다.
...
NOTE: -0과 NaN 등을 조심해야 하는 건 앞서 설명한 == 알고리즘과 비슷하다.

그러나 < 비교 대상이 모두 문자열 값이면, 각 문자를 단순 어휘(알파벳 순서로) 비교한다.
...
두 배열을 ToPrimitive로 강제변환하면 문자열이기 때문에 a, b 는 숫자로 강제변환하지 않는다. 따라서 "42"와 "043"을 문자 단위로 비교한다. "0"은 "4" 보다 작은 값이므로 false가 된다.
...
결과는 false다. 둘 모두 [object Object]로 변환되어 명백히 a 가 b 보다 어휘비교로 작지 않다.
> 책에는 "a도 [object Object], b도 [object Object]로 변환되어 어휘적인 비교를 할 수 없기 때문이다." 라고 적혀있었다. 이해가 안되서 찾아봤는데 원서에는 "because a becomes [object Object] and b becomes [object Object], and so clearly a is not lexicographically less than b." 라고 적혀있었다. 번역이 좀 애매했던 것 같다. 어휘적인 비교를 할 수 없다고 해서 이해가 잘 안됐었는데(혹시 타입이 다른건가? 왜 할 수가 없지?) 둘 다 강제변환으로 문자열이 됐고 lexicographic 비교시 같기 때문에 false가 됐다고 생각하면 될 것 같다.
...
a == b 는 왜 true가 아닐까? 둘 다 동일한 문자열("[object Object]")이니까... 놉. ==는 두 객체가 똑같은 값에 대한 래퍼런스일 경우 동등함.

그런데 a < b , a ==  b , a > b 모두 false인데 a <= b 와 a >= 는 어떻게 true?

*a <= b 는 실제로 b < a의 평가 결과를 부정하도록 명세에 기술되어 있다.* 그래서 b < a 가 false 이기 때문에 a <= b 는 true가 된다.

자바스크립트 엔진은 a <= b 를 '더 크지 않은'(!(a > b)) -> *!(b < a)* 의미로 해석한다. 더구나 a >= b 는 먼저 b <= a로 재해석한 다음 동일한 추론을 적용한다.

불행히도 '엄격한 관계 비교'는 없다. a < b 같은 관계 비교 과정에서 암시적 강제변환을 원천 봉쇄할 수 없다.

그러니 조심해서 관계 비교를 해야할 것 같은 상화에서는 관계 비교를 하기 전, 비교할 값들을 명시적으로 강제변환해두는 편이 안전한다.
...
강제변환이라고 하는 자바스크립트의 타입변환의 작동 원리를 명시적/암시적 두 가지 유형으로 나누어 알아보았다.

강제변환은 많은 욕을 먹고 있지만 알고 보면 많은 경우 꽤 유용하다. 그러니 적절한 판단을하여 결정하기 위해 강제변환에 대해 꼼꼼히 학습해두자.

명시적 강제변환은 다른 타입의 값으로 반환하는 의도가 확실한 코드를 말하며 혼동의 여지를 줄이고 코드 가독성 및 유지 보수성을 높일 수 있는 장점이 있다.

암시적 강제변환은 '숨겨진' 로직에 의한 부수 효과가 있으며 타입변환이 처리되는 과정이 명확하지 않다. 그래서 나쁘다고들 하지만 오히려 코드 가독성을 향상하는 장점도 있다.
